...
type okStore struct{ ids []string; items interface{}; total int }
...
func TestListGPUsHandlerSuccessAndErrors(t *testing.T) {
    ...
    if body.Total != 4 || body.Page != 2 || body.Limit != 2 { t.Fatalf("unexpected body: %+v", body) }

    // limit=0 falls back to default via parser; expect OK
    req2 := httptest.NewRequest(http.MethodGet, "/gpus?limit=0", nil)
    rr2 := httptest.NewRecorder()
    h.ListGPUs(rr2, req2)
    if rr2.Result().StatusCode != http.StatusOK { t.Fatalf("expected 200 for limit=0 fallback, got %d", rr2.Result().StatusCode) }
    ...
    if rr3.Result().StatusCode != http.StatusInternalServerError { t.Fatalf("expected 500 for store error") }
}
...
func TestQueryGPUTelemetryHandler(t *testing.T) {
    ...
    if rr.Result().StatusCode != http.StatusBadRequest { t.Fatalf("expected 400 for missing gpu id") }

    // invalid start_time
    req2 := httptest.NewRequest(http.MethodGet, "/gpus/g1/telemetry?start_time=bad", nil)
    rr2 := httptest.NewRecorder()
    h.QueryGPUTelemetry(rr2, req2)
    if rr2.Result().StatusCode != http.StatusBadRequest { t.Fatalf("expected 400 for invalid time") }
    ...
    if rr4.Result().StatusCode != http.StatusOK { t.Fatalf("expected 200 for success") }
    var body QueryResponse
    if err := json.NewDecoder(rr4.Result().Body).Decode(&body); err != nil { t.Fatalf("decode: %v", err) }
    if body.Total != 1 || body.Page != 1 { t.Fatalf("unexpected body: %+v", body) }
}
...
func TestMiddlewares(t *testing.T) {
    ...
    if rr.Result().StatusCode != http.StatusCreated { t.Fatalf("expected 201, got %d", rr.Result().StatusCode) }
    ...
    if rr2.Result().StatusCode != http.StatusUnauthorized { t.Fatalf("expected 401, got %d", rr2.Result().StatusCode) }
}
